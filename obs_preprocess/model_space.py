...
class ModelSpace( object ):
    ...
    def __init__( self, METCRO3D, METCRO2D, CONC, date_range ):
        ...
        #generate lat/lon limits for first pass obs filter
        # first generate edges
        xEdge = self.gridmeta[ 'XCELL' ] * np.arange( self.ncol +1)+\
            xoffset -0.5*self.gridmeta['XCELL']
        yEdge = self.gridmeta[ 'YCELL' ] * np.arange( self.nrow +1)+\
            yoffset -0.5*self.gridmeta['YCELL']
