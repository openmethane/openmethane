#!/usr/bin/env bash
# Archives the results from a monthly run to an S3 bucket
#
# This is intended to be run as part of the production deployment of the monthly workflow
...
# If the run was successful the data is stored in the prefix `results/${DOMAIN_NAME}/monthly/${YEAR}/${MONTH}`.
...
#
# This will overwrite any existing monthly output with the same domain, year and month.
...
if [[ "$SUCCESS" == "true" ]]; then
  echo "Monthly run successful"

  YEAR=$(date -u +"%Y" -d $START_DATE)
  MONTH=$(date -u +"%m" -d $START_DATE)

  PREFIX=results/${DOMAIN_NAME}/monthly/${YEAR}/${MONTH}
