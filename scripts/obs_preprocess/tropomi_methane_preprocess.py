...
def run_tropomi_preprocess(source, output_file, qa_cutoff):
    """
    ...
    filelist = sorted([os.path.realpath(f) for f in glob.glob(source)])
    ...
    for fname in filelist:
        ...
        with Dataset(fname, "r") as f:
            ...
            averaging_kernel = np.reshape(
                ch4_averaging_kernel, (-1, ch4_averaging_kernel.shape[-1])
            )
