    ...
    for idomain, domain in enumerate(domains):
        ...
        for i in range(0, emi_lat.shape[0] - 1):
            for j in range(0, emi_lat.shape[1] - 1):
                ...
                ind_lon = np.asarray(
                    np.where((down_lon <= maxlon) & (down_lon >= minlon))
                ).squeeze()
                ind_lat = np.asarray(
                    np.where((down_lat <= maxlat) & (down_lat >= minlat))
                ).squeeze()
                ...
                if (ind_lat.size > 0) & (ind_lon.size > 0):
                    ...
                    # print('hello')
                    unit_factor = 1000.0 / 16.0
                    # area_jooj=arealatlon(minlat,minlon,maxlat,maxlon)
                    emi_grdcell[i, j] = (
                        down_ch4[minindlat:maxindlat, minindlon:maxindlon].sum() * unit_factor
                    )
        ...
        cmaqspec = "CH4"
        outvars = dict()
        outvars["TFLAG"] = grdem.createVariable(
            "TFLAG",
            "i4",
            (
                "TSTEP",
        ...
        outvars[cmaqspec] = grdem.createVariable(
            cmaqspec,
            "f4",
            ("TSTEP", "LAY", "ROW", "COL"),
            zlib=True,
        ...
        emi_grdcell = random.uniform(low=0.0, high=emi_grdcell.max(), size=emi_grdcell.shape)
