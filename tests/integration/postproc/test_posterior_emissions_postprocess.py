...
def test_posterior_emissions_postprocess(target_environment, test_data_dir):
    ...
    assert posterior_multipliers.emis["CH4"].max() == pytest.approx(1.1112205), (
        "input values (max) do not match expected"
    )
    assert posterior_multipliers.emis["CH4"].sum() == pytest.approx(99.69608), (
    ...
    assert prior_emis_mean_surf.max() == pytest.approx(95.388885), (
        "prior emissions values (max) do not match expected"
    )
    assert prior_emis_mean_surf.mean() == pytest.approx(4.111149), (
        "prior emissions values (mean) do not match expected"
    )
    assert prior_emis_mean_surf[0][0] == pytest.approx(1.8064358234405518), (
    ...
    assert posterior_emissions["ch4"].sum() == pytest.approx(6.25249e-08), (
    ...
    assert posterior_emissions["ch4"][0][0][0][0] == expected[0][0], (
        "emissions do not equal multiplier times prior"
    )
    assert posterior_emissions["ch4"][0][0][2][4] == expected[2][4], (
        "emissions do not equal multiplier times prior"
    )
    assert posterior_emissions["ch4"][0][0][4][4] == expected[4][4], (
        "emissions do not equal multiplier times prior"
    )
    ...
    assert posterior_emissions["lat"][0, 0].item() == pytest.approx(-23.729095458984375), (
        "lat coordinates do not match expected"
    )
    assert posterior_emissions["lat"][2, 2].item() == pytest.approx(-23.524276733398438), (
        "lat coordinates do not match expected"
    )
    assert posterior_emissions["lat"][4, 4].item() == pytest.approx(-23.31928062438965), (
        "lat coordinates do not match expected"
    )

    assert posterior_emissions["lon"][0, 0].item() == pytest.approx(148.247802734375), (
        "lon coordinates do not match expected"
    )
    assert posterior_emissions["lon"][2, 2].item() == pytest.approx(148.4224853515625), (
        "lon coordinates do not match expected"
    )
    assert posterior_emissions["lon"][4, 4].item() == pytest.approx(148.5965576171875), (
        "lon coordinates do not match expected"
    )
    ...
    prior_sector_vars = [
        var_name for var_name in prior_emissions.variables if var_name.startswith("ch4_sector")
    ]
    for sector_var in prior_sector_vars:
        sector_mean = prior_emissions[sector_var].mean(axis=0)
        # spot check a single coord at 1,1
        assert float(sector_mean[0, 1, 1]) == float(
            posterior_emissions[f"prior_{sector_var}"][0, 0, 1, 1]
        )
...
def test_posterior_emissions_postprocess_multi_day(target_environment, test_data_dir):
    ...
    # spot check several cells
    assert posterior_emissions["ch4"][0, 0, 0, 0].item() == pytest.approx(2.8897012560591406e-10), (
        "emissions do not match expected"
    )
    assert posterior_emissions["ch4"][0, 0, 4, 4].item() == pytest.approx(8.876147039593718e-11), (
        "emissions do not match expected"
    )
