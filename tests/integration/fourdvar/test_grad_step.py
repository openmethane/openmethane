...
def _run_grad_step():
    ...
    initCost = (
        0.5
        * (
            ((np.array(simul.value) - np.array(observed.value)) / np.array(observed.uncertainty))
            ** 2
        ).sum()
    )
    ...
    pertObsCost = (
        ...
        * (
            (
                (np.array(pert_simul.value) - np.array(observed.value))
                / np.array(observed.uncertainty)
            )
            ** 2
        ).sum()
    )
