...
class Plane:
    ...
    def isUp(self, point):
        """Check if a point is on the "up" side of a plane.

        i.e in the direction of rather than opposed to the defined normal.

        Allow a slight user-defined tolerance for the common case
        where the point lies in the plane of self.
        """
...
class Polyhedron:
    """Defined as a list of planes with no requirement that they form a closed shape.

    e.g. an open square tube is valid.
    """
    ...
    def intersectionPrismVolume(self, corners):
        """Volume of intersection between self and a prism defined by its corners."""
    ...
    def montecarloVolume(self, corners):
        """Use montecarlo sampling to estimate the volume of intersection between prism and self.

        The prism is defined using the corners.
        """
        vMin = np.array(corners[0]).squeeze()
        vMax = np.array(corners[1]).squeeze()
