...
| `compare_arms.py` | Decision criterion 3 of [#248](https://github.com/openmethane/openmethane/issues/248): pairs two experiment arms' `simulobs.pic.gz` by (time, latitude, longitude) and fits a trend to the daily mean of their difference. The arms share everything but the one rewrite, so this is the most precise of the measurements. |
