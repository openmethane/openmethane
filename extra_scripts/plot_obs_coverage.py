...
cov = plt.pcolormesh(coverage, vmin=0, cmap="Blues", edgecolor="k", linestyle=":", linewidth=0.1)
cbar = plt.colorbar()
...
plt.hist(obs_step, bins=[i + 0.5 for i in range(nday * nstep + 1)])
plt.xlim(0, nday * nstep + 1)
plt.xticks(
    [nstep / 2 + nstep * i for i in range(nday)],
